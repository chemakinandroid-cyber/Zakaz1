...
export function formatDateTime(value) {
  if (!value) return "";
  return new Date(value).toLocaleString("ru-RU", { timeZone: LOCAL_TIMEZONE });
}
export const styles = {
  ...
  badgeWarn: { display:"inline-block", padding:"4px 10px", borderRadius:999, background:"#fef3c7", color:"#92400e", fontSize:12 },
  branchBtn:(a)=>({ borderRadius:18, border:a?"1px solid #0f172a":"1px solid #e2e8f0", background:a?"#0f172a":"#fff", color:a?"#fff":"#0f172a", padding:16, textAlign:"left", cursor:"pointer" }),
  categoryBtn:(a)=>({ borderRadius:999, border:"1px solid #cbd5e1", background:a?"#0f172a":"#fff", color:a?"#fff":"#0f172a", padding:"8px 12px", cursor:"pointer", whiteSpace:"nowrap" }),
