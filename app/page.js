...
export default function Home() {
  ...
  const cancelOrder = (orderId) => {
    setOrders((prev) =>
      prev.map((o) => (o.id === orderId && ["new", "accepted"].includes(o.status) ? { ...o, status: "canceled" } : o))
    );
  };

  const repeatOrder = (order) => {
    setSelectedBranchId(order.branchId);
    setCart(order.items.map((x) => ({ ...x, cartId: `${x.itemId}-${Date.now()}-${Math.random()}` })));
  };
  ...
  const toggleStop = (itemId) => {
    setStopList((prev) => {
      ...
      return {
        ...prev,
        [selectedBranchId]: current.includes(itemId) ? current.filter((x) => x !== itemId) : [...current, itemId],
      };
    });
  };
            ...
            <div style={styles.card}>
              ...
              {cart.length === 0 ? (
                ...
                <>
                    ...
                    {cart.map((item) => (
                      <div key={item.cartId} style={styles.cartItem}>
                        <div style={styles.row}>
                          ...
                          <button onClick={() => updateQty(item.cartId, -item.qty)} style={{ ...styles.button, ...styles.lightBtn, padding: "6px 10px" }}>
                            ×
                          </button>
                        </div>
                  ...
                  <button style={{ ...styles.button, ...styles.darkBtn, width: "100%", marginTop: 12 }} disabled={!branchOrderAllowed} onClick={placeOrder}>
                    Оформить заказ
                  </button>
                </>
              )}
