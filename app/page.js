...
const branches = [
  { id: "konechnaya", name: "На Виражах — Конечная", address: "ул. Конечная, 10/4", open: "10:00", close: "22:00", cutoff: "21:30" },
  { id: "airport", name: "На Виражах — Аэропорт", address: "п. Аэропорт, 7", open: "10:00", close: "21:00", cutoff: "20:30" },
];
...
function orderAllowed(branch) {
  const now = new Date();
  return now.getHours() * 60 + now.getMinutes() <= getMinutes(branch.cutoff);
}

function getDrinkSuggestion(branchId, stopList) {
  const branchDrinks = items.filter(
    (x) =>
      x.category === "Напитки" &&
      x.branches.includes(branchId) &&
      !stopList[branchId]?.includes(x.id) &&
      !x.comingSoon
  );
...
function getSuggestions(item, branchId, stopList) {
  ...
  if (item.category === "Блюда во фритюре") {
    return [
      available("sauce-cheese") || available("sauce-ketchup") || available("sauce-sweet-mustard"),
      drink,
    ].filter(Boolean);
  }
...
export default function Home() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") localStorage.setItem(STORAGE_KEYS.branch, JSON.stringify(selectedBranchId));
  }, [selectedBranchId]);

  useEffect(() => {
    if (typeof window !== "undefined") localStorage.setItem(STORAGE_KEYS.stopList, JSON.stringify(stopList));
  }, [stopList]);

  useEffect(() => {
    if (typeof window !== "undefined") localStorage.setItem(STORAGE_KEYS.orders, JSON.stringify(orders));
  }, [orders]);
  ...
  const branchItems = useMemo(
    () => items.filter((item) => item.branches.includes(selectedBranchId)),
    [selectedBranchId]
  );
  ...
  const openItem = (item) => {
    if (stopList[selectedBranchId]?.includes(item.id)) return;
  ...
  const cancelOrder = (orderId) => {
    setOrders((prev) =>
      prev.map((o) =>
        o.id === orderId && ["new", "accepted"].includes(o.status)
          ? { ...o, status: "canceled" }
          : o
      )
    );
  ...
  const repeatOrder = (order) => {
    ...
    setCart(
      order.items.map((x) => ({
        ...x,
        cartId: `${x.itemId}-${Date.now()}-${Math.random()}`,
      }))
    );
    setMobileCartOpen(true);
  };

  const toggleStop = (itemId) => {
    setStopList((prev) => {
      const current = prev[selectedBranchId] || [];
      return {
        ...prev,
        [selectedBranchId]: current.includes(itemId)
  ...
  const currentSuggestions = cart.length
    ? getSuggestions(
        items.find((i) => i.id === cart[cart.length - 1].itemId) || { category: "" },
        selectedBranchId,
        stopList
      )
    : [];
  ...
  const rightColumn = (
    <>
      <div style={styles.card}>
        ...
        {cart.length === 0 ? (
          ...
          <>
                ...
                <div key={item.cartId} style={styles.cartItem}>
                  <div style={styles.row}>
                    ...
                    <button
                      ...
                      style={{ ...styles.button, ...styles.lightBtn, padding: "6px 10px" }}
                    >
                      ×
                    </button>
                  </div>
                ...
                <div style={{ display: "grid", gap: 8 }}>
                  {currentSuggestions.slice(0, 3).map((s) => (
                    <div key={s.id} style={{ ...styles.row, border: "1px solid #e2e8f0", borderRadius: 16, padding: 10 }}>
                      ...
                      <button
                        style={{ ...styles.button, ...styles.darkBtn, padding: "8px 12px" }}
            ...
            <button
              style={{ ...styles.button, ...styles.darkBtn, width: "100%", marginTop: 12 }}
              onClick={placeOrder}
              disabled={!branchOrderAllowed}
            >
              Оформить заказ
            </button>
          </>
            ...
            {branchOrders.map((order) => (
              <div key={order.id} style={styles.cartItem}>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
                  {!adminMode && (
                    <button style={{ ...styles.button, ...styles.lightBtn }} onClick={() => repeatOrder(order)}>
                      Повторить
                    </button>
                  )}

                  {!adminMode && ["new", "accepted"].includes(order.status) && (
                    <button style={{ ...styles.button, ...styles.dangerBtn }} onClick={() => cancelOrder(order.id)}>
                      Отменить
                    </button>
                  )}
      ...
      <div style={{ ...styles.card, marginTop: 16 }}>
        <h3 style={{ marginTop: 0 }}>{adminMode ? "Заготовка админки" : "Комментарий к заказу"}</h3>
        {adminMode ? (
          <div style={{ display: "grid", gap: 8, color: "#475569" }}>
            <div>• В этом режиме уже можно ставить стоп по выбранной точке.</div>
            <div>• Можно менять статусы заказов.</div>
            <div>• Следующий шаг — вынести всё в Supabase и подключить реальную многопользовательскую работу.</div>
          </div>
        ) : (
          <div style={{ color: "#64748b" }}>
            В следующей версии подключим реальный чат и комментарии к заказу.
          </div>
        )}
      </div>
    </>
    ...
    <div style={styles.page}>
      <div style={styles.wrap}>
        <div style={styles.top}>
          ...
          <button
            style={{ ...styles.button, ...(adminMode ? styles.darkBtn : styles.lightBtn) }}
            ...
            <div style={styles.card}>
              <div style={styles.row}>
                <div>
                  ...
                  <div style={{ color: "#64748b" }}>
                    Меню, заказы и стоп-лист учитывают выбранную точку
                  </div>
                </div>
                ...
                {branches.map((b) => (
                  <button key={b.id} onClick={() => setSelectedBranchId(b.id)} style={styles.branchBtn(selectedBranchId === b.id)}>
                    ...
                    <div style={{ marginTop: 8, opacity: 0.8 }}>
                      {b.open}–{b.close} · заказы до {b.cutoff}
                    </div>
                  </button>
                ))}
              </div>
              ...
              <div style={{ display: "flex", gap: 8, overflowX: "auto", marginTop: 16, paddingBottom: 4 }}>
                {categories.map((category) => (
                  <button
                    ...
                    style={styles.categoryBtn(activeCategory === category)}
                  >
                    {category}
                  </button>
                ))}
                ...
                return (
                  <div key={item.id} style={styles.itemCard}>
                    ...
                    <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
                      <button
                        style={{ ...styles.button, ...styles.darkBtn, flex: 1 }}
                        disabled={stopped || item.comingSoon || !branchOrderAllowed}
        ...
        <div style={{ position: "fixed", right: 16, bottom: 16, zIndex: 20 }}>
          <button
            style={{ ...styles.button, ...styles.darkBtn, boxShadow: "0 8px 24px rgba(15,23,42,.18)" }}
            onClick={() => setMobileCartOpen(true)}
          >
            Корзина · {fmt(cartTotal)}
          </button>
        </div>

        {mobileCartOpen && (
          <div style={styles.modalBg} onClick={() => setMobileCartOpen(false)}>
            <div style={{ ...styles.modal, maxWidth: 700 }} onClick={(e) => e.stopPropagation()}>
              <div style={{ ...styles.row, marginBottom: 12 }}>
                <h3 style={{ margin: 0 }}>Корзина и заказы</h3>
                <button style={{ ...styles.button, ...styles.lightBtn }} onClick={() => setMobileCartOpen(false)}>Закрыть</button>
              </div>
              {rightColumn}
            </div>
          </div>
          ...
          <div style={styles.modalBg} onClick={() => setSelectedItem(null)}>
            <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
              <div style={{ ...styles.row }}>
                ...
                <button style={{ ...styles.button, ...styles.lightBtn }} onClick={() => setSelectedItem(null)}>
                  Закрыть
                </button>
              </div>

              <p style={{ color: "#64748b", marginTop: 8 }}>
                Выберите добавки. Общая цена пересчитывается автоматически.
              </p>
                      ...
                      <input
                        ...
                        onChange={() => {
                          setSelectedAddons((prev) =>
                            checked ? prev.filter((a) => a.id !== addon.id) : [...prev, addon]
                          );
                        }}
                      />
                    </label>
              ...
              <button
                style={{ ...styles.button, ...styles.darkBtn, width: "100%", marginTop: 16 }}
                onClick={() => selectedItem && addToCart(selectedItem, selectedAddons)}
              >
                Добавить в корзину
              </button>
            </div>
