  ...
  return (
    <div style={styles.page}>
      <div style={styles.wrap}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "minmax(0, 1.15fr) minmax(360px, .85fr)", gap: 16 }}>
          <div>
            <div style={styles.card}>
              <div style={{ ...styles.row, alignItems: isMobile ? "flex-start" : "center", flexDirection: isMobile ? "column" : "row" }}>
                ...
                <div style={{ textAlign: isMobile ? "left" : "right" }}>
                  <div style={{ fontSize: 16, color: "#94a3b8" }}>Заказы до {branch.cutoff}</div>
              ...
              {filteredItems.map((item) => (
                <div key={item.id} style={styles.itemCard}>
                  <div style={styles.row}>
                    <div>
                      <div style={{ fontSize: 22, fontWeight: 700 }}>{item.name} {item.spicy ? "🌶" : ""}</div>
                  ...
                  {item.description && <div style={{ marginTop: 12, color: "#cbd5e1", fontSize: 16 }}>{item.description}</div>}
            ...
            <div style={{ ...styles.card, position: isMobile ? "static" : "sticky", top: 16 }}>
              ...
              {cart.length === 0 ? (
                ...
                <>
                      ...
                      <div key={item.cartId} style={styles.cartItem}>
                        <div style={styles.row}>
                          <div>
                            ...
                            {item.addons.length > 0 && <div style={{ marginTop: 6, color: "#94a3b8", fontSize: 16 }}>{item.addons.map((a) => a.name + " (+" + fmt(a.price) + ")").join(", ")}</div>}
                          </div>
                          <button onClick={() => updateQty(item.cartId, -item.qty)} style={{ ...styles.button, ...styles.lightBtn, padding: "8px 12px" }}>×</button>
                        </div>

                        <div style={{ ...styles.row, marginTop: 12 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            <button style={{ ...styles.button, ...styles.lightBtn, padding: "8px 12px" }} onClick={() => updateQty(item.cartId, -1)}>-</button>
                            <span>{item.qty}</span>
                            <button style={{ ...styles.button, ...styles.lightBtn, padding: "8px 12px" }} onClick={() => updateQty(item.cartId, 1)}>+</button>
                          </div>
                      ...
                      <div style={{ display: "grid", gap: 8 }}>
                        {currentSuggestions.map((s) => (
                          <div key={s.id} style={{ ...styles.row, border: "1px solid #1f2937", borderRadius: 16, padding: 10, background: "#0b1220" }}>
                            ...
                            <button style={{ ...styles.button, ...styles.darkBtn, padding: "14px 16px" }} onClick={() => addSuggestionToCart(s)}>+ {fmt(s.price)}</button>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div style={{ ...styles.row, marginTop: 16, paddingTop: 16, borderTop: "1px solid #1f2937", fontSize: 22, fontWeight: 700 }}>
                    <span>Итого</span>
                    ...
                    <div key={order.id} style={styles.cartItem}>
                      <div style={{ ...styles.row, alignItems: isMobile ? "flex-start" : "center", flexDirection: isMobile ? "column" : "row" }}>
                        <div>
                          ...
                          <div style={{ color: "#94a3b8", fontSize: 16 }}>{order.branchName}</div>
                        </div>
                        <StatusBadge status={order.status} />
                      </div>
                      <div style={{ marginTop: 8, color: "#94a3b8", fontSize: 16 }}>{order.createdAt}</div>
        ...
        {selectedItem && (
          <div style={styles.modalBg} onClick={() => setSelectedItem(null)}>
            <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
                  ...
                  return (
                    <label key={addon.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", border: "1px solid #1f2937", borderRadius: 16, padding: 12, cursor: "pointer" }}>
                      <div>
                        ...
                        <div style={{ fontSize: 16, color: "#94a3b8" }}>+{fmt(addon.price)}</div>
                      </div>
              ...
              <div style={{ ...styles.row, marginTop: 16, fontSize: 22, fontWeight: 700 }}>
                <span>Итого</span>
