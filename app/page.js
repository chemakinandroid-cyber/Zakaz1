...
const items = [
  {
    ...
    name: "Шаурма «На Виражах»",
    category: "Шаурма в лаваше",
    variant: "курица",
    price: 260,
    spicy: false,
    branches: ["konechnaya", "airport"],
    description:
      "Курица, помидоры, огурцы, пекинская капуста, фирменный соус",
    addonGroup: "shawarma-chicken",
  },
  {
    ...
    branches: ["konechnaya", "airport"],
    description:
      "Свинина, помидоры, огурцы, пекинская капуста, фирменный соус",
    addonGroup: "shawarma-pork",
  },
  {
    id: "shawarma-caesario",
    name: "Шаурма «Цезарио»",
    category: "Шаурма в лаваше",
    variant: "курица",
    price: 290,
    ...
    branches: ["konechnaya", "airport"],
    description:
      "Курица, помидоры, пекинская капуста, твердый сыр, фирменный соус, сухарики",
    addonGroup: "shawarma-chicken",
  },
  {
    id: "shawarma-redjet-chicken",
    name: "Шаурма «Ред.Джет»",
    category: "Шаурма в лаваше",
    variant: "курица",
    price: 290,
    spicy: true,
    branches: ["konechnaya", "airport"],
    description:
      "Курица, огурцы, помидоры, пекинская капуста, фирменный и острый соусы, халапеньо",
    addonGroup: "shawarma-chicken",
  },
  {
    ...
    branches: ["konechnaya", "airport"],
    description:
      "Свинина, огурцы, помидоры, пекинская капуста, фирменный и острый соусы, халапеньо",
    addonGroup: "shawarma-pork",
  },
...
const initialStopList = {
  konechnaya: [
    "lemonade-cherry-yuzu",
    "lemonade-mango-maracuya",
    "lemonade-watermelon-basil",
  ],
  airport: [
    "lemonade-cherry-yuzu",
    "lemonade-mango-maracuya",
...
function getSuggestions(item, branchId, stopList) {
  ...
  if (item.category === "Блюда во фритюре") {
    return [
      available("sauce-cheese") ||
        available("sauce-ketchup") ||
        available("sauce-sweet-mustard"),
      drink,
    ].filter(Boolean);
  }
...
export default function Home() {
  ...
  const newOrders = useMemo(
    () => branchOrders.filter((o) => o.status === "new"),
    [branchOrders]
  );

  const activeOrders = useMemo(
    () => branchOrders.filter((o) => ["accepted", "preparing", "ready"].includes(o.status)),
    [branchOrders]
  );

  const finishedOrders = useMemo(
    () => branchOrders.filter((o) => ["completed", "canceled", "no_show"].includes(o.status)),
    [branchOrders]
  );
              ...
              >
                {branches.map((b) => (
                  <button
                    ...
                    style={styles.branchBtn(selectedBranchId === b.id)}
                  >
                  ...
                  <div key={item.id} style={styles.itemCard}>
                    ...
                    {item.comingSoon && (
                      <div style={{ marginTop: 12 }}>
                        <span style={styles.badge}>Скоро в продаже</span>
                      </div>
                    )}

                    {stopped && (
                      <div style={{ marginTop: 12 }}>
                        <span style={styles.badgeDanger}>Стоп</span>
                      </div>
                    )}
          ...
          <div>
            ...
            <div style={{ ...styles.card, marginTop: 16 }}>
              <h3 style={{ marginTop: 0 }}>
                {adminMode ? "Управление заказами" : "Мои заказы"}
              </h3>
              ...
              {!adminMode ? (
                ...
                <div style={{ display: "grid", gap: 16 }}>
                  <div>
                    <h4 style={{ margin: "0 0 8px" }}>Новые — ждут звонка клиента</h4>
                    {newOrders.length === 0 ? (
                  ...
                  <div>
                    <h4 style={{ margin: "0 0 8px" }}>Подтверждённые и активные</h4>
                        ...
                        {activeOrders.map((order) => (
                          <div key={order.id} style={styles.cartItem}>
                            ...
                            >
                              ...
                              <button
                                style={{ ...styles.button, ...styles.dangerBtn }}
                                onClick={() => setOrderStatus(order.id, "no_show")}
                              >
                                Не забран
                              </button>
                            </div>
                  ...
                  <div>
                    <h4 style={{ margin: "0 0 8px" }}>Завершённые</h4>
