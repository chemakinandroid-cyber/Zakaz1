...
function TimerChip({ mins, status }) {
  if (["completed", "canceled", "no_show"].includes(status)) {
    return <Chip bg="#1f2937">закрыт</Chip>;
  }
...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    const c2 = supabase
      .channel("admin-items-" + selectedBranchId)
  ...
  useEffect(() => {
    ...
    if (
      audioEnabled &&
      audioUnlocked &&
      previousNew.current !== 0 &&
      newCount > previousNew.current
    ) {
      playBeepStrong();
    }
  ...
  }, [orders, audioEnabled, audioUnlocked]);
  ...
  if (loading) {
    return <div style={{ minHeight: "100vh", padding: 20, background: C.bg }}>Загрузка...</div>;
  }
  ...
  const block = (title, rows, activeActions) => (
    ...
    >
      ...
      {rows.length === 0 ? (
        <div style={{ marginTop: 12, color: C.muted }}>Нет заказов.</div>
      ) : (
        <div style={{ marginTop: 12, display: "grid", gap: 12 }}>
          {rows.map((o) => (
            <div
              key={o.id}
              style={{ background: C.panel2, borderRadius: 22, padding: 14 }}
            >
              <div
                style={{
                  display: "flex",
              ...
              >
                <div>
                  ...
                  {o.confirmed_at && (
                    <div style={{ marginTop: 4, color: C.muted, fontSize: 14 }}>
                      Подтверждён: {formatDateTime(o.confirmed_at)}
                    </div>
                  )}
                </div>
              ...
              {o.note && (
                <div style={{ marginTop: 8, color: C.muted, fontSize: 14 }}>{o.note}</div>
              )}
  ...
  return (
    <div
      style={{ minHeight: "100vh", background: C.bg, padding: mobile ? 14 : 18 }}
      onClickCapture={armAudio}
      onTouchStartCapture={armAudio}
    >
      <div style={{ maxWidth: 1320, margin: "0 auto" }}>
        ...
        >
          <div>
            <div style={{ fontSize: mobile ? 34 : 40, fontWeight: 900, lineHeight: 1 }}>
              Админка
            </div>
            <div style={{ color: C.muted, marginTop: 6 }}>
              Крупные блоки, локальное время устройства, таймер от подтверждения
            </div>
          </div>
